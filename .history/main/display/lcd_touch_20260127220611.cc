...
void LcdTouch::touch_driver_read(lv_indev_t *drv, lv_indev_data_t *data) {
    ...
    if (err == ESP_OK && touch_cnt > 0 && touch_cnt <= TOUCH_MAX_POINT) {
        ...
        // When bypass_gesture_ is true (menu visible), only detect long press to close menu
        // Skip swipe and tap detection so LVGL can handle button clicks
        if (bypass_gesture_) {
            ESP_LOGD(TAG, "Bypass mode: touch at (%d, %d)", current_x, current_y);
            ...
            if (!long_press_detected_) {
                ...
                if (touch_duration > gesture_config_.long_press_time_us) {
                    ...
                    ESP_LOGI(TAG, "🖐️ Long press detected (menu mode) at (%d, %d)", current_x, current_y);
                    if (gesture_callback_) {
                        gesture_callback_(TOUCH_GESTURE_LONG_PRESS, current_x, current_y);
                    }
                }
            }
