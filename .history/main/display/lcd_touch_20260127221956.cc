...
void LcdTouch::touch_driver_read(lv_indev_t *drv, lv_indev_data_t *data) {
    ...
    // ============================================================
    // BYPASS MODE: When menu is visible, detect long press and TAP
    // ============================================================
    if (bypass_gesture_) {
        if (touch_cnt > 0) {
            ...
            if (!was_touching_) {
                ...
                touch_start_x_ = point_data[0].x;
                touch_start_y_ = point_data[0].y;
                long_press_detected_ = false;
                ESP_LOGI(TAG, "Bypass: touch start at (%d, %d)", point_data[0].x, point_data[0].y);
            }
            
            // Detect long press to close menu
            if (!long_press_detected_) {
                int64_t duration = esp_timer_get_time() - touch_start_time_;
                if (duration > gesture_config_.long_press_time_us) {
                    long_press_detected_ = true;
                    ESP_LOGI(TAG, "Bypass: long press detected");
                    if (gesture_callback_) {
                        gesture_callback_(TOUCH_GESTURE_LONG_PRESS, point_data[0].x, point_data[0].y);
                    }
                }
            }
            was_touching_ = true;
            ...
            data->state = LV_INDEV_STATE_RELEASED;
            if (was_touching_ && !long_press_detected_) {
                // TAP detected - short touch without long press
                int64_t duration = esp_timer_get_time() - touch_start_time_;
                if (duration < gesture_config_.long_press_time_us) {
                    ESP_LOGI(TAG, "Bypass: TAP detected at (%d, %d), duration=%lld us", 
                             touch_start_x_, touch_start_y_, duration);
                    if (gesture_callback_) {
    ...
    // ============================================================
    // NORMAL MODE: Full gesture detection
    // ============================================================
...
void LcdTouch::SetBypassGesture(bool bypass) {
    bypass_gesture_ = bypass;
    ESP_LOGI(TAG, "Bypass gesture set to %d", bypass);
}
