...
#include "../features/theme/clock_face_manager.h"
 ...
 * 
 * Screen modes:
 * - IDLE: Weather/time display
 * - CLOCK: Clock face with swipe navigation
 * - MENU: Settings menu grid (6 buttons)
 * - AGENT: Agent selector with swipe navigation
 * - LUNAR: Lunar calendar display
 * - LISTENING/SPEAKING: AI mode
 */

// Screen modes
enum ScreenMode {
    SCREEN_MODE_IDLE = 0,       // Default idle screen (weather/time)
    SCREEN_MODE_CLOCK,          // Clock face display
    SCREEN_MODE_MENU,           // Settings menu
    SCREEN_MODE_AGENT_SELECT,   // Agent selection screen
    SCREEN_MODE_LUNAR,          // Lunar calendar
...
// Clock face change callback (simple version for gesture controller)
typedef void (*SimpleClockFaceChangeCallback)(int direction);  // direction: -1 left, +1 right
...
public:
    ...
    void SetClockFaceManager(ClockFaceManager* mgr) { clock_face_manager_ = mgr; }
    void SetLunarCalendar(LunarCalendar* lunar) { lunar_calendar_ = lunar; }
    
    // Set callbacks
    void SetVolumeCallback(VolumeChangeCallback callback) { volume_callback_ = callback; }
    void SetClockFaceCallback(SimpleClockFaceChangeCallback callback) { clock_face_callback_ = callback; }
...
private:
    ...
    ClockFaceManager* clock_face_manager_ = nullptr;
    LunarCalendar* lunar_calendar_ = nullptr;
    
    // Callbacks
    VolumeChangeCallback volume_callback_ = nullptr;
    SimpleClockFaceChangeCallback clock_face_callback_ = nullptr;
    ...
    int clock_face_count_ = 4;       // Total clock faces available
