...
void LcdTouch::touch_driver_read(lv_indev_t *drv, lv_indev_data_t *data) {
    ...
    // ============================================================
    // BYPASS MODE: When menu is visible, let LVGL handle touches
    // Only detect long press to close menu
    // ============================================================
    if (bypass_gesture_) {
        if (touch_cnt > 0) {
            ...
            // Track for long press detection only
            if (!was_touching_) {
            ...
            data->state = LV_INDEV_STATE_RELEASED;
            if (was_touching_) {
                ESP_LOGI(TAG, "Bypass: touch released");
            }
            was_touching_ = false;
            long_press_detected_ = false;
        }
    ...
    if (err == ESP_OK && touch_cnt > 0 && touch_cnt <= TOUCH_MAX_POINT) {
        ...
        // In bypass mode, just reset was_touching_ for next touch
        if (bypass_gesture_) {
            if (was_touching_) {
                ESP_LOGD(TAG, "Bypass mode: touch released");
                was_touching_ = false;
                long_press_detected_ = false;
            }
            data->continue_reading = true;
