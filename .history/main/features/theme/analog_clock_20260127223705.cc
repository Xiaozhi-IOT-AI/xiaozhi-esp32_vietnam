...
AnalogClock::AnalogClock(lv_obj_t* parent, int x, int y, int diameter)
    ...
    // Store screen dimensions for fullscreen background
    lv_obj_t* scr = lv_scr_act();
    screen_width_ = lv_obj_get_width(scr);
    screen_height_ = lv_obj_get_height(scr);
...
void AnalogClock::CreateUI() {
    // Create fullscreen background container to cover entire screen
    clock_root_ = lv_obj_create(parent_);
    lv_obj_set_size(clock_root_, screen_width_, screen_height_);
    lv_obj_set_pos(clock_root_, 0, 0);  // Cover entire screen from top-left
    lv_obj_set_style_bg_color(clock_root_, ThemeManager::HexToLvColor(theme_.background_color), 0);
    lv_obj_set_style_bg_opa(clock_root_, LV_OPA_COVER, 0);
    ...
    // Create canvas for drawing clock face (centered in the container)
    ...
    // Center the canvas within the fullscreen container
    lv_obj_set_pos(canvas_, x_, y_);
    
    // Initial draw
    UpdateTime();
    
    ESP_LOGI(TAG, "Analog clock created: fullscreen %dx%d, clock face %dx%d at (%d,%d)", 
             screen_width_, screen_height_, diameter_, diameter_, x_, y_);
}
