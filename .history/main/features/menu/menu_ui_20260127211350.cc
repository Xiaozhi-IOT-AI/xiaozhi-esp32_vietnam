...
void MenuUI::CreateUI() {
    ...
    int cols = 2;
    int rows = 3;
    int available_width = width_ - (2 * padding) - ((cols - 1) * gap);
    int available_height = height_ - (2 * padding) - ((rows - 1) * gap);
    ...
    for (int i = 0; i < MENU_ACTION_COUNT; i++) {
        ...
        // Make clickable - ensure click events are handled properly
        ...
        if (icons_[i]) {
            ...
            // Make icon not clickable so parent button receives the click
        ...
        if (labels_[i]) {
            ...
            // Make label not clickable so parent button receives the click
