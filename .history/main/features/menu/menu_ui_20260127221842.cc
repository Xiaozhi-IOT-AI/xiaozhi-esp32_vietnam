...
void MenuUI::CreateUI() {
    ...
    lv_obj_remove_flag(menu_root_, LV_OBJ_FLAG_CLICKABLE);  // Don't capture clicks - let buttons handle them
    ...
    for (int i = 0; i < MENU_ACTION_COUNT; i++) {
        ...
        // Save button rect for direct touch handling
        button_rects_[i] = {(int16_t)x, (int16_t)y, (int16_t)btn_width, (int16_t)btn_height};
        ESP_LOGI(TAG, "Button %d rect: (%d,%d,%d,%d)", i, x, y, btn_width, btn_height);
...
void MenuUI::OnButtonPressed(lv_event_t* e) {
    lv_obj_t* current_target = static_cast<lv_obj_t*>(lv_event_get_current_target(e));
    int action = (int)(intptr_t)lv_obj_get_user_data(current_target);
    ESP_LOGI(TAG, "Button PRESSED: %d (%s)", action, default_items_[action].name);
}
...
// Direct touch handling - check which button contains the point
int MenuUI::GetButtonAtPoint(int16_t x, int16_t y) {
    for (int i = 0; i < MENU_ACTION_COUNT; i++) {
        const auto& r = button_rects_[i];
...
// Handle touch directly - call this from board when menu is visible
bool MenuUI::HandleTouch(int16_t x, int16_t y) {
    if (!is_visible_) return false;
    
    int btn_idx = GetButtonAtPoint(x, y);
    if (btn_idx >= 0 && btn_idx < MENU_ACTION_COUNT) {
        ESP_LOGI(TAG, "Direct touch on button %d (%s) at (%d,%d)", 
                 btn_idx, default_items_[btn_idx].name, x, y);
        
        if (callbacks_[btn_idx]) {
            ESP_LOGI(TAG, "Executing callback for button %d", btn_idx);
            callbacks_[btn_idx](callback_data_[btn_idx]);
            return true;
        }
    }
    return false;
