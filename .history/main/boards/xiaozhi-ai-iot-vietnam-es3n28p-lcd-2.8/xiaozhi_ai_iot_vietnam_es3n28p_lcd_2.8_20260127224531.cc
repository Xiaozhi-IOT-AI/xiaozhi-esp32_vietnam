 ...
 private:
  ...
  void InitializeTouch() {
    ...
    touch_->SetGestureCallback([this](TouchGesture gesture, int16_t x, int16_t y) {
      ESP_LOGI(TAG, "Touch gesture detected: %d at (%d, %d), menu=%d, clock=%d, info=%d", 
               static_cast<int>(gesture), x, y, menu_visible_, clock_visible_, info_visible_);
      
      // When info page is visible, TAP to dismiss (swipe handled by LVGL scroll)
      if (info_visible_) {
        if (gesture == TOUCH_GESTURE_TAP || gesture == TOUCH_GESTURE_LONG_PRESS) {
          ESP_LOGI(TAG, "TAP/LongPress - hiding info page");
          this->HideInfoPage();
        }
        // Don't process other gestures - let LVGL handle scroll
        return;
      }
