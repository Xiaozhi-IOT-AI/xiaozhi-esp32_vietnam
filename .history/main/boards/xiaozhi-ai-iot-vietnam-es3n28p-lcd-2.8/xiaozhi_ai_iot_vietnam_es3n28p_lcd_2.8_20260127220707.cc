 ...
 private:
  ...
  void InitializeTouch() {
    ...
    touch_->SetGestureCallback([this](TouchGesture gesture, int16_t x, int16_t y) {
      ESP_LOGI(TAG, "Touch gesture detected: %d at (%d, %d), menu_visible=%d", 
               static_cast<int>(gesture), x, y, menu_visible_);
      
      // When menu is visible, only handle long press to hide menu
      // TAP events should be handled by LVGL buttons
      if (menu_visible_) {
        if (gesture == TOUCH_GESTURE_LONG_PRESS) {
          ESP_LOGI(TAG, "Long press - hiding menu");
          this->HideMenu();
        }
        // For TAP, don't process here - LVGL should handle button clicks
        // But LVGL needs the touch data, so we don't block it
        return;
      }
  ...
  void InitializeMenu() {
    ...
    menu_ui_->SetCallback(MENU_ACTION_CLOCK, [](void* data) {
      ...
      auto* board = static_cast<XiaozhiAIIoTEs3n28p*>(data);
      board->HideMenu();
      // TODO: Implement clock face switching
      board->GetDisplay()->SetChatMessage("system", "🕐 Chức năng đồng hồ\nĐang phát triển...");
    }, this);
    ...
    menu_ui_->SetCallback(MENU_ACTION_RADIO, [](void* data) {
      ...
      auto* board = static_cast<XiaozhiAIIoTEs3n28p*>(data);
      board->HideMenu();
      board->GetDisplay()->SetChatMessage("system", "📻 Radio FM\nĐang phát triển...");
    }, this);
    ...
    menu_ui_->SetCallback(MENU_ACTION_LUNAR, [](void* data) {
      ...
      LunarDate lunar = LunarCalendar::SolarToLunar(
      ...
      snprintf(msg, sizeof(msg), 
        "🌙 Âm lịch hôm nay\n\n"
        "Ngày %d tháng %d năm %s\n"
        "Năm %s\n"
        "%s",
        lunar.day, lunar.month,
        lunar.leap_month ? "(nhuận)" : "",
        LunarCalendar::GetYearName(lunar.year).c_str(),
        LunarCalendar::GetDayInfo(lunar).c_str()
      );
